        ...
        <div class="content container-fluid">
            ...
            <div class="page-header d-flex justify-content-between align-items-center">
                <div class="page-title">
                    <h5 class="mb-0 fw-semibold">Products Listings / {{ $locationData->location_name }}</h5>
                ...
                <div class="btn-group">
                    ...
                    @auth
                        @if (auth()->user()->hasPermission('category_create'))
                            <a href="{{ route('products.subaccount.add') }}" class="btn btn-sm btn-primary ms-2">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        ...
                        <input type="text" id="searchInput" class="form-control mb-3"
                            placeholder="Search for products..." onkeyup="searchTable()">
                        ...
                        <table class="table table-center table-hover datatable" id="myTable">
                            ...
                            <tbody>
                                @forelse ($products as $index => $data)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $data->locationProducts->name }}</td>
                                        <td>{{ $data->locationProducts->sku }}</td>
                                        <td>{{ $data->locationProducts_category->name ?? 'N/A' }}</td>
                                        <td>{{ $data->locationProducts_unit->name ?? 'N/A' }}</td>
                                        <td>{{ $businessInfo->currency ?? 'N/A' }}{{ number_format($data->locationProducts->cost_price, 2) }}
                                        </td>
                                        <td>{{ $businessInfo->currency ?? 'N/A' }}{{ number_format($data->locationProducts->sale_price, 2) }}
                                        </td>
                                        ...
                                        <td>{{ $data->locationProducts_vendors->vendor_name ?? 'N/A' }}</td>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    <a class="dropdown-item"
                                                        href="{{ route('product.serials.show', Crypt::encrypt($data->id)) }}"><i
                                                            class="far fa-folder-closed me-2"></i> Products Keys</a>
                                                    <a class="dropdown-item"
                                                        href="{{ route('products.edit.show', Crypt::encrypt($data->id)) }}"><i
