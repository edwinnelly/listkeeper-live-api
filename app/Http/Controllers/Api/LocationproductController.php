...
public function index()
{
    try {
        ...
        if (!$user) {
            Log::warning('LocationProductList index: Unauthenticated access attempt');
        ...
        if (empty($user->active_business_key)) {
            Log::warning('LocationProductList index: No active business selected', [
                'user_id' => $user->id
            ]);
        ...
        $products = LocationProductList::where('business_key', $user->active_business_key)
            ->get()
        ...
        Log::error('LocationProductList index failed', [
            'error' => $e->getMessage(),
            'file' => $e->getFile(),
            'line' => $e->getLine(),
